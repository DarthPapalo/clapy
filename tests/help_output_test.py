...
def rich_to_ansi(markup: str) -> str:
    ...
    console.print(markup, highlight=False, end="") # No end since we add it the assert statement
...
def test_inherit_help_print_method(capsys: pytest.CaptureFixture[str]):
    ...
    captured= capsys.readouterr()
    assert captured.out == (
        "Inherited print argument~ Usage: my-app subcmd\n"
        "\n"
    )


def test_rich_style(capsys: pytest.CaptureFixture[str]):
    my_style = ClapyRichStyle(arg_help="cyan i", title="green b", usage="green b i", subcommand_help="red")
